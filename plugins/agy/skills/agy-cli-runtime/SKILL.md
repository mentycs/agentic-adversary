...
- You may use the `gpt-5-4-prompting` skill to rewrite the user's request into a tighter Antigravity prompt before the single `task` call.
- That prompt drafting is the only Claude-side work allowed. Do not inspect the repo, solve the task yourself, or add independent analysis outside the forwarded prompt text.
- Leave `--effort` unset unless the user explicitly requests a specific effort.
- Leave model unset by default. Add `--model` only when the user explicitly asks for one.
- Map `spark` to `--model gpt-5.3-agy-spark`.
...
- If the forwarded request includes `--model`, normalize `spark` to `gpt-5.3-agy-spark` and pass it through to `task`.
...
Safety rules:
- Default to write-capable Antigravity work in `agy:agy-rescue` unless the user explicitly asks for read-only behavior.
